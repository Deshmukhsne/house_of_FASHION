...
public function get_all_products_for_drycleaning()
    {
        $this->db->select('order_id, productName');
        $this->db->from('orders'); // Change 'orders' to your table name
        $this->db->where('status !=', 'Returned'); // Optional: exclude already returned items
